    ...
    where T : UnityEngine.Texture
{
    ...
    public List<UnityEngine.Texture2D> FaceTexture = new List<UnityEngine.Texture2D>();
    ...
    protected virtual Mat MatFromTexture(UnityEngine.Texture2D texture, OpenCvSharp.Unity.TextureConversionParams texParams)
    {
    ...
    /// <param name="texParams">Texture parameters (flipped, rotated etc.)</param>
    protected virtual void ImportTexture(UnityEngine.Texture2D texture, OpenCvSharp.Unity.TextureConversionParams texParams)
    {
    ...
    public virtual void ProcessTexture(UnityEngine.Texture2D texture, OpenCvSharp.Unity.TextureConversionParams texParams, bool detect = true)
    {
        ...
        if (detect)
        {
            ...
            Mat gray = new Mat();
            //Cv2.Rotate(processingImage, temp, RotateFlags.Rotate180);
    ...
    /// </summary>
    public List<UnityEngine.Texture2D> MarkDetected(bool drawSubItems = true)
    {
        FaceTexture.Clear();
        // mark each found eye
        foreach (DetectedFace face in Faces)
        {
            ...
            Mat faceImg = new Mat(Image, face.Region);
            // UnityEngine.Texture2D texture = new UnityEngine.Texture2D(500, 500, UnityEngine.TextureFormat.RGB24, false);
            try
            {
                texture = OpenCvSharp.Unity.MatToTexture(faceImg, texture);
                FaceTexture.Add(texture);
            }
            catch (Exception e)
            {
                continue;
    ...
    where T : UnityEngine.Texture
{
    ...
    /// <param name="texParams">Texture parameters (flipped, rotated etc.)</param>
    /// <param name="detect">Flag signalling whether we need detection on this frame</param>
    public override void ProcessTexture(UnityEngine.Texture2D texture, OpenCvSharp.Unity.TextureConversionParams texParams, bool detect = true)
    {
